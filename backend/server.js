...
app.get("/api/trading/positions", requireTerminalAuth, async (req,res) => {
  try {
    ...
    try {
      ...
      const pendingAfter=await ref.collection("positions").where("status","==","pending").get();
      pendingOrders=pendingAfter.docs.map(d=>({id:d.id,symbol:d.data()?.symbol,name:MARKET_SYMBOLS[d.data()?.symbol]?.name||d.data()?.symbol,orderType:d.data()?.orderType,lot:d.data()?.lot,entryPrice:d.data()?.entryPrice,stopLoss:d.data()?.stopLoss||null,takeProfit:d.data()?.takeProfit||null,status:"pending",createdAt:d.data()?.openedAt||null}));
    } catch (refreshError) {
